<template>
  <v-row :dense="true">
    <v-col>
      <icon-btn icon="mdi-eye-outline" color="primary" :disabled="disabled || usedHint || !gameStore.playing" @click="showAnimeHint()">
        Show Anime Hint
      </icon-btn>
    </v-col>
  </v-row>
...
function showAnimeHint() {
  socket.emit(SOCKET_EVENTS.GAME_USE_LIFE_LINE, 'ANIME_HINT');
}
